...
trip_data <- here("data",
  ...
  select(HOUSEID,
         ...
         R_AGE,
         EDUC,
         WTTRDFIN)  # Keep relevant columns
```
...
TDTRPNUM - Incrementing travel day trip number
...
trip_data |>
  ...
  select(`Number of trips (weighted)`,
...
Filtering for household-level variables:
...
n_seniors <- person_data |>
  ...
  group_by(HOUSEID) |>
  summarise(n_seniors = sum(is_senior)) # construct seniors

n_children <- person_data %>%
  mutate(is_child = R_AGE < 4) %>%
  group_by(HOUSEID) %>%
  summarise(n_children = sum(is_child)) # construct children
...
person_data <- person_data %>%
  left_join(social_recreational_trips) 
```

### Merge trip data with person data

```{r}
social_recreational_trips <- trip_data |>
  filter(purpose == "HBSR") |>
  group_by(HOUSEID, PERSONID) |>
  summarise(social_recreational_trips = n())



colnames(person_data)
```

## Data Visualization

```{r}
ggplot(social_recreational_trips) +
  geom_histogram(aes(x = social_recreational_trips),
                 binwidth = 1,
                 color = "gray",
                 fill = "lightgreen") +
  scale_x_continuous(name = "Number of home-based social-recreational trips",
                     breaks = seq(0, 12, by=1)) +
  scale_y_continuous(name = "Number of perople in sample") +
  theme_minimal()
```
...
```{r}
social_recreational_trips |>
  summarise(`Average count of HBSR trips` = mean(social_recreational_trips),
            `Standard deviation` = sd(social_recreational_trips)) |>
  kable(digits = 3)
```
...
```{r}
hbsr_model_1 <- lm(social_recreational_trips ~ n_seniors + n_children + WRKCOUNT + veh_avail + income, 
   data = social_recreational_trips)

summary(hbsr_model_1)
```
